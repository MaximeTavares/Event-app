 ...
 * @return {*}  {(string | null)}
 */
export function buildMapStatusMessage(
	params: BuildMapStatusMessageParams,
): string | null {
	...
	if (
		(params.isCityGeocodingError || params.isCityNotFound) &&
		!params.hasEffectiveOrigin
	) {
 ...
 * @return {*}  {(string | null)}
 */
export function buildMapWarningMessage(
	params: BuildMapWarningMessageParams,
): string | null {
 ...
 * @return {*}  {(string | null)}
 */
export function buildListStatusMessage(
	params: BuildListStatusMessageParams,
): string | null {
	...
	if (params.isEventsError) {
		return (
			params.eventsErrorMessage ?? 'Impossible de charger les événements.'
		);
	}
